...
A workspace owns tabs and panes. Its sidebar state rolls up from the agents inside it, so you can see which project needs attention. In the compact sidebar, the first emoji in the workspace name replaces its number and status icon. A workspace with just one pane running Neovim shows its Nerd Font icon instead. Names without emoji keep the usual number and status icon. The emoji or Neovim icon is bright when an agent is blocked or has finished unseen work, and faded otherwise.
